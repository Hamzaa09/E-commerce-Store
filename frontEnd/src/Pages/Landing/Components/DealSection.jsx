...
const DealSection = () => {
  ...
  const { dealsProducts, productLoading } = useSelector(
    (state) => state.productSlice
  );
  useEffect(() => {
    const response = dispatch(getDealsProductsThunk({ deals: true }));
  }, []);
  ...
  return (
    ...
    >
      ...
      {/* items  */}
      <div className="owl-carousel deal-section owl-theme w-full flex justify-around hover:cursor-pointer">
      ...
      {/* buttons */}
      <button
        onClick={() => $(".deal-section").trigger("prev.owl.carousel")}
        className={`${
      ...
      <button
        onClick={() => $(".deal-section").trigger("next.owl.carousel")}
        className={`${
          buttondisplay ? "visible" : "hidden"
